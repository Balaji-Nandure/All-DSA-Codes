...
class MatrixChainMultiplicationTabulation {
public:
    // TC: O(n^3), SC: O(n^2) for dp array
    int matrixMultiplication(int N, vector<int>& arr) {
        // dp[i][j] = minimum cost to multiply matrices from i to j
        vector<vector<int>> dp(N, vector<int>(N, 0));
        
        // Fill table diagonally (by length of chain)
        // len = length of chain (number of matrices)
...
// 4. Space Optimization (Not applicable - range DP requires O(n^2) space)
// Note: Range DP problems typically require O(n^2) space and cannot be optimized further
// because we need to access all subranges.
class Solution {
public:
    // TC: O(n^3), SC: O(n^2) for dp array
    int matrixMultiplication(int N, vector<int>& arr) {
